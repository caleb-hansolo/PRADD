...
//     // The fileData now comes from UploadFrame after chunk upload completes
//     if (fileType === 'dataset') {
...
const TrainingPage = ({ sessionId }) => {
  ...
  // Periodic refresh to ensure thumbnails are synchronized
  useEffect(() => {
    if (sessionId) {
  ...
  const fetchSessionData = async () => {
    try {
      ...
      if (data.thumbnails) {
        ...
        // Clear thumbnails if not available
        setDatasetThumbnail(null);
        setMirrorThumbnail(null);
        setPatternThumbnail(null);
      }
